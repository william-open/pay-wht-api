...
type SettleSnapshot struct {
	...
	MerchantRecv   float64 `json:"merchantRecv"`
	UpstreamCost   float64 `json:"upstreamCost"`
	AgentIncome    float64 `json:"agentIncome"`
	PlatformProfit float64 `json:"platformProfit"`
}
...
type MerchantOrder struct {
	...
	Amount         float64        `gorm:"column:amount;type:decimal(18,4);not null" json:"amount"`              // 订单金额
	Fees           float64        `gorm:"column:fees;type:decimal(10,2);not null" json:"fees"`                  // 手续费
	PayAmount      float64        `gorm:"column:pay_amount;type:decimal(18,4);not null" json:"payAmount"`       // 实际支付金额
	RealMoney      float64        `gorm:"column:real_money;type:decimal(18,4);not null" json:"realMoney"`       // 实际到账金额
	FreezeAmount   float64        `gorm:"column:freeze_amount;type:decimal(18,4);not null" json:"freezeAmount"` // 冻结金额
	...
	MRate          *float64       `gorm:"column:m_rate;type:decimal(10,2)" json:"mRate"`                        // 商户费率
	UpRate         *float64       `gorm:"column:up_rate;type:decimal(10,2)" json:"upRate"`                      // 上游通道费率
	Country        *string        `gorm:"column:country;type:varchar(30)" json:"country"`                       // 国家
	UpFixedFee     *float64       `gorm:"column:up_fixed_fee;type:decimal(10,2)" json:"upFixedRate"`            // 上游通道固定费用
	MFixedFee      *float64       `gorm:"column:m_fixed_fee;type:decimal(10,2)" json:"MFixedFee"`               // 商户通道固定费用
